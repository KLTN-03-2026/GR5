...
export async function GET(request: Request) {
  try {
    ...
    const sanPham1 = await prisma.san_pham.create({
      data: {
        ten_san_pham: `Cà Chua Cherry Đà Lạt (TEST ${Date.now()})`,
        ma_danh_muc: danhMuc.id,
        bien_the_san_pham: {
          create: [
            { ma_sku: `SKU-CCH-500G-${Date.now()}`, ten_bien_the: 'Hộp 500g', don_vi_tinh: 'Hộp', gia_ban: 25000, gia_goc: 15000 },
            { ma_sku: `SKU-CCH-1KG-${Date.now()}`, ten_bien_the: 'Thùng 1kg', don_vi_tinh: 'Thùng', gia_ban: 45000, gia_goc: 28000 }
          ]
        }
      },
    ...
    const sanPham2 = await prisma.san_pham.create({
      data: {
        ten_san_pham: `Rau Xà Lách Thủy Canh (TEST ${Date.now()})`,
        ma_danh_muc: danhMuc.id,
        bien_the_san_pham: {
          create: [
            { ma_sku: `SKU-RXL-250G-${Date.now()}`, ten_bien_the: 'Túi 250g', don_vi_tinh: 'Túi', gia_ban: 15000, gia_goc: 8000 }
          ]
        }
      },
      include: { bien_the_san_pham: true }
    ...
    const b1 = sanPham1.bien_the_san_pham[0].id;
    const b2 = sanPham1.bien_the_san_pham[1].id;
    const b3 = sanPham2.bien_the_san_pham[0].id;
    ...
    return NextResponse.json({
      ...
      poId: phieuNhap.id,
      products: [sanPham1.ten_san_pham, sanPham2.ten_san_pham]
    });
