...
const statusMeta: Record<string, { label: string; color: string; bg: string; icon: any }> = {
  CHO_GIAO_HANG:{ label: "Chờ giao hàng", color: "text-amber-700",  bg: "bg-amber-50",  icon: Clock },
  CHO_KIEM_TRA: { label: "KH kiểm tra",  color: "text-blue-700",   bg: "bg-blue-50",   icon: Eye },
  DA_DUYET:     { label: "Đã xong",       color: "text-green-700",  bg: "bg-green-50",  icon: CheckCircle },
...
// ─── MAIN COMPONENT ────────────────────────────────────────────────
export default function PurchaseOrderCreation({ formOptions }: { formOptions: any }) {
  ...
  // Load phiếu list
  const loadPhieus = useCallback(async (page = currentPage) => {
    const res = await fetch(`/api/admin/warehouse/import?status=all&page=${page}&limit=15`);
  ...
  const submitForReview = async (phieuId: number) => {
    showToast("success", "Đơn đã được lưu!");
    loadPhieus();
    setActiveTab("list");
  };
  ...
  return (
    <div className="space-y-5 relative">
      ...
      {activeTab === "form" && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-gray-50/50">
            ...
            <div>
              <h2 className="font-bold text-gray-800 text-base">Tạo Đơn Đặt Hàng Mới</h2>
              <p className="text-xs text-gray-400">Đơn hàng ở trạng thái Chờ Giao Hàng để Nv.Kho tiếp nhận</p>
            </div>
          </div>
          ...
          {step === "success" ? (
            <div className="p-8 text-center space-y-4">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-3 text-sm text-amber-700 flex items-start gap-2 text-left max-w-md mx-auto">
                ...
                <span>Đơn hàng đang ở trạng thái <strong>CHỜ GIAO HÀNG</strong>. Nhân viên kho sẽ nhìn thấy đơn này để nhập kho khi NCC giao tới.</span>
              </div>
              <div className="flex gap-3 justify-center">
                <button onClick={() => { setStep("form"); setCreatedPhieu(null); setActiveTab("list"); loadPhieus(); }}
                  className="px-5 py-2.5 bg-[#1D9E75] text-white text-sm font-bold rounded-xl hover:bg-[#158a63] transition-colors flex items-center gap-2">
                  <ChevronRight size={14} /> Xem danh sách đơn
                </button>
      ...
      {activeTab === "list" && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h2 className="font-bold text-gray-800 text-sm flex items-center gap-2"><Eye size={16} className="text-blue-500" /> Danh sách Đơn Đặt Hàng</h2>
                  ...
                  phieuList.map((p) => {
                    ...
                    return (
                      <tr key={p.id} className="hover:bg-gray-50/40 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right text-xs font-bold text-gray-700">{(p as any).tong_so_luong || 0}</td>
